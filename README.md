...
Whether to use `aqtinstall` to install Qt using its official online installer enabling the commercial versions for those owning a license.
...
```yml
    - name: Install Qt
      uses: Kidev/install-qt-action@v4.3.3
      with:
        version: '5.15.3'
        target: 'desktop'
...
#### `email`
If `use-official` is true, will use this email to authenticate with Qt servers
